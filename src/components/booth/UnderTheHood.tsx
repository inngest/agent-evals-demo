 ...
 */
export function UnderTheHood({ snippet }: { snippet: HighlightedBoothSnippet }) {
  return (
    <aside className="booth-drawer grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] bg-[#17131a] text-white">
      <header className="grid gap-2 border-b border-white/15 px-10 pb-6 pt-12">
        ...
        <div className="display text-[34px] font-semibold">{snippet.label}</div>
        <p className="text-[21px] leading-[1.4] text-white/70">{snippet.description}</p>
      </header>
      <div
        className="code-html min-h-0 overflow-hidden"
        style={{ "--code-font-size": "19px" } as React.CSSProperties}
      >
