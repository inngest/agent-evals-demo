...
- Booth positioning (Sept 2026): primary tagline "Unbreakable agents,
  ...
  Evaluation / Sandboxes (beta). Story is a loop: Run -> Observe -> Evaluate.
...
- Legacy surfaces are preserved at `/research`, `/booth-story`, and
  ...
  surfaces.
- SDK upgraded to `inngest@4.20.0` (sandbox requirement). Experimental eval
...
| --- | --- | --- |
| Loop demo shell | `/` renders tagline, pillars, stage nav (Run/Observe/Evaluate), code pane synced per stage | Locally ready |
...
| Evaluate stage | Feedback -> `/api/research/signal`; bakeoff -> `/api/research/experiment`; Scores/Experiment deep links | Locally ready, Cloud pending |
| Loop code snippets | `src/content/loop-snippets.ts` (run/observe/evaluate) with highlight markers; legacy snippets untouched | Locally ready |
...
| Talk track | `docs/demo-talk-track.md` rewritten for Run/Observe/Evaluate with event placeholders | Draft ready, stakeholder review pending |
